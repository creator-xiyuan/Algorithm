...
# 第六章——优先队列（堆）
虽然发送到打印机的作业一般被放到队列中，但这未必总是最好的做法。例如，可能有一项作业特别重要，或者同时有多项单页的作业和一项100页的作业等待打印。
这类特殊的应用需要特殊的队列，称之为**优先队列**

### 6.1 模型
优先队列至少允许Insert和DeleteMin操作（入队和出队）
...
#### 2.堆序性质
最小元应该在根上且任意子树的根节点页应小于其所有后代

#### 3.堆的基本操作
`见代码实现`

#### 4.其他的堆操作
对于小根堆，虽然求最小值操作可以在常数时间完成，但对于求最大值却毫无帮助。事实上，一个堆所蕴含的关于序的信息很少。
如果我们假设通过其他方法可以得知每一个元素的位置，那么下面三种操作的最坏运行时间均为$O(log N)$：DecreaseKey、IncreaseKey、Delete

BuildHeap(H)操作的一般实现方法如下，其复杂度为$O(N)$
```C
for(i = N / 2; i > 0; i --)
    PercolateDown(i);
```
